...
from buffers import audio_queue, SR
...
def _estimate_bpm_from_file(y: np.ndarray, sr: int,
                             logger: Logger) -> float | None:
...
def load_track(path: str, state: PhantomState,
               logger: Logger) -> tuple[np.ndarray, float, float]:
    """
